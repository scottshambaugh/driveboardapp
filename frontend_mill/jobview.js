...
function jobview_resize() {
  ...
  var max_canvas_height =
    $(window).innerHeight() - nav_height_init - footer_height_init;
  ...
  if (app_config_main !== undefined) {
    ...
    if (aspect_canvas_void > aspect_workspace) {
      // workspace is narrower
    ...
    } else if (aspect_workspace > aspect_canvas_void) {
      // workspace is wider
...
function jobview_ready() {
  ...
  jobview_camera = new THREE.PerspectiveCamera(
    70,
  ...
  // Controls
  jobview_controls = new THREE.OrbitControls(
    jobview_camera,
    jobview_renderer.domElement,
  );
