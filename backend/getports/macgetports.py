...
corefound.CFStringGetCString.argtypes = [
    ctypes.c_void_p,
    ctypes.c_char_p,
    ctypes.c_int,
    ctypes.c_int,
...
def getstring(dev, key):
    cf = iokit.IORegistryEntryCreateCFProperty(
        dev, key, kCFDefaultAllocator, kNilOptions
    )  # get from device object
    buf = ctypes.create_string_buffer(MAXSTRLEN)  # c string (byte array)
    corefound.CFStringGetCString(
        cf, buf, MAXSTRLEN, kCFStringEncodingASCII
    )  # convert from CFString to c string
...
def portiter():
    ...
    while True:
        ...
        if not ds:  # a null pointer means we've exhausted the iterator
            break
        yield (
            getstring(ds, kIOTTYDeviceKey),
            getstring(ds, kIODialinDeviceKey),
        )  # device name and dialin address
