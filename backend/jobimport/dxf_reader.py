...
class DXFReader:
    ...
    def parse(self, dxfstring):
        ...
        while 1:
            ...
            if self.line == "LINE":
                self.do_line()
            elif self.line == "CIRCLE":
                self.do_circle()
            elif self.line == "ARC":
                self.do_arc()
            elif self.line == "LWPOLYLINE":
                self.do_lwpolyline()
            elif self.line == "SPLINE":
                self.complain_spline()
            elif self.line == "ENDSEC":
                break
            else:
                self.complain_invalid()
    ...
    def addArc(self, path, x1, y1, rx, ry, phi, large_arc, sweep, x2, y2):
        ...
        def _angle(u, v):
            a = math.acos(
                (u[0] * v[0] + u[1] * v[1])
                / math.sqrt(((u[0]) ** 2 + (u[1]) ** 2) * ((v[0]) ** 2 + (v[1]) ** 2))
            )
            sgn = -1
            if u[0] * v[1] > u[1] * v[0]:
                sgn = 1
