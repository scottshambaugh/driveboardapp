function controls_ready() {
  ...
  $("#export_btn").click(function (e) {
    if (!jobhandler.isEmpty()) {
      ...
      var blob = new Blob([jobhandler.getJson("\t")], {
  ...
  */
  ...
  $("#moveBy_btn").click(function (e) {
    if (!$(this).hasClass("disabled")) {
      ...
      $(".tool_extra_btn").hide();
      x_mm =
        Math.round(parseFloat(document.getElementById("x_input").value) * 10) /
        10;
      y_mm =
        Math.round(parseFloat(document.getElementById("y_input").value) * 10) /
        10;
      if (isNaN(x_mm)) {
        x_mm = 0;
      }
      if (
        ...
        x_mm =
          Math.round((-status_cache.pos[0] - status_cache.offset[0]) * 10) / 10;
      }
      if (isNaN(y_mm)) {
        y_mm = 0;
      }
      if (
        ...
        y_mm =
          Math.round((-status_cache.pos[1] - status_cache.offset[1]) * 10) / 10;
      }

      request_relative_move(
        x_mm,
        y_mm,
        0,
        app_config_main.seekrate,
